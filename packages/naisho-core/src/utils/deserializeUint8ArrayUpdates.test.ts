...
test("should deserialize a single Uint8Array correctly", () => {
  ...
  const serialized = serializeUint8ArrayUpdates([array]);
  const deserialized = deserializeUint8ArrayUpdates(serialized);
...
test("should deserialize multiple Uint8Arrays correctly", () => {
  ...
  const serialized = serializeUint8ArrayUpdates([array1, array2]);
  const deserialized = deserializeUint8ArrayUpdates(serialized);
...
test("should throw an error if the serialized string is invalid", () => {
  expect(() => deserializeUint8ArrayUpdates("invalid")).toThrowError();
});
